...
class mkcss_helper {
  ...
  protected function detect_iofile() {
    // check input file
    if (!$this->check_command_line()) {
      $this->check_web_access();
    }
    // make output file
    $mkfile = $this->mkfile;
    $cssfile = preg_replace('/\.yaml$/', '', $mkfile);
    $cssfile = preg_replace('/\.mkcss$/', '.css', $cssfile);
  ...
  protected function check_command_line() {
    global $argv;
    if (!isset($argv)) return false;
    // args length
    if (count($argv) == 1) {
  ...
  protected function generate() {
    // load & generate
    $mtime = date("Y-m-d H:i:s",filemtime($this->mkfile));
    $src = @file_get_contents($this->mkfile);

    $parser = new mkcss_parser();
    $css = $parser->compile($src);

    $f = basename($this->cssfile);
